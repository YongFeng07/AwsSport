...
- Register/login/logout, account page, dark/light mode, password visibility toggle,
  TARUMT faculty dropdown, server/client validation preventing future birth dates,
  arithmetic CAPTCHA, and optional Google OAuth 2.0 login.
...
## CAPTCHA and Google login

Password registration and login include a session-backed arithmetic CAPTCHA. No
external CAPTCHA service or key is required. The answer is checked on the server
and consumed after one attempt.

Google login is hidden until all three Google OAuth variables are configured.
Create an OAuth 2.0 Web application in Google Cloud, add the exact callback URL
as an authorized redirect URI, then copy `.env.example` to `.env` and set:
...
`migration_google_login.sql` once. Fresh installations already contain the
required nullable password and unique Google subject columns in `schema.sql`.
The PHP cURL extension must be enabled on the web server.
