...
$stmt = $conn->prepare('SELECT name, email, id_number, faculty, date_of_birth, password_hash FROM users WHERE id = ?');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['form'] ?? '') === 'profile') {
    ...
    if ($name === '' || $date_of_birth === '' || $id_number === '' || $faculty === '') {
        ...
        $profileError = 'Date of birth must be a valid date and cannot be in the future.';
    } else {
...
<label>Date of Birth <span class="required-mark">*</span> <input type="date" name="date_of_birth" value="<?= htmlspecialchars($user['date_of_birth'] ?? '') ?>" max="<?= date('Y-m-d') ?>" required></label>
<button type="submit">Save Changes</button>
</form>
</div>

<div class="form-card">
<h2><?= $user['password_hash'] === null ? 'Set Password' : 'Change Password' ?></h2>
